...
float freq_list[16] = {
    88.0, 88.5, 88.9, 89.1, 89.3, 89.7, 90.0, 90.5,
    90.9, 91.1, 91.3, 91.7, 92.0, 92.5, 92.9, 93.1
};
...
void list_widgets(lv_obj_t* parent) {
    ...
    lv_obj_set_size(list, 460, 280);
    lv_obj_align_to(list, freq_input, LV_ALIGN_OUT_BOTTOM_MID, 0, 100);

    static int32_t col_dsc[] = { 150, 150, 150, 150, LV_GRID_TEMPLATE_LAST };
    static int32_t row_dsc[] = { 80, 80, 80, 80, LV_GRID_TEMPLATE_LAST };
    ...
    for (int i = 0; i < 16; i++) {
        ...
        lv_obj_add_event_cb(list_btns[i], freq_list_event_cb, LV_EVENT_CLICKED, &freq_list[i]);
        lv_obj_set_grid_cell(list_btns[i], LV_GRID_ALIGN_CENTER, i % 4, 1, LV_GRID_ALIGN_CENTER, i / 4, 1);
    }
...
void lv_lc_widgets(void) {
    ...
    lv_obj_align_to(freq_up_btn, freq_input, LV_ALIGN_BOTTOM_MID, 150, 80);
    ...
    lv_obj_align_to(freq_scan_btn, freq_input, LV_ALIGN_BOTTOM_MID, 0, 80);
    ...
    lv_obj_add_event_cb(freq_down_btn, freq_down_event_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_align_to(freq_down_btn, freq_input, LV_ALIGN_BOTTOM_MID, -150, 80);
